...
const { query, queryOne, newId, parseJ } = require('../src/lib/db');
...
router.get('/children', auth, authorize(['parent']), async (req, res) => {
  try {
    const children = await Student.find({ parentIds: req.userId }).populate('classId', 'name section').populate('schoolId', 'name');
    res.json(children);
...
router.get('/child/:studentId/feed', auth, authorize(['parent']), async (req, res) => {
  try {
    const activities = await ActivityLog.find({ studentId: req.params.studentId }).sort({ date: -1 }).limit(50).populate('teacherId', 'firstName lastName');
    const attendance = await Attendance.findOne({ studentId: req.params.studentId, date: { $gte: new Date().setHours(0, 0, 0, 0) } });
    res.json({ activities, todayAttendance: attendance });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

router.get('/child/:studentId/report/:month/:year', auth, authorize(['parent']), async (req, res) => {
  try {
    const report = await Report.findOne({ studentId: req.params.studentId, month: parseInt(req.params.month), year: parseInt(req.params.year) }).populate('teacherId', 'firstName lastName');
...
router.get('/messages/:teacherId', auth, authorize(['parent']), async (req, res) => {
  try {
    const messages = await Message.find({ $or: [{ senderId: req.userId, recipientId: req.params.teacherId }, { senderId: req.params.teacherId, recipientId: req.userId }] }).sort({ createdAt: -1 }).populate('senderId', 'firstName lastName').populate('recipientId', 'firstName lastName');
    res.json(messages);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.post('/ptm/book', auth, authorize(['parent']), async (req, res) => {
  try {
    ...
    ptm.status = 'booked'; ptm.parentId = req.userId; ptm.bookedOn = new Date();
    await ptm.save();
...
router.get('/ptm/slots/:teacherId', auth, authorize(['parent']), async (req, res) => {
  try {
    const slots = await PTMBooking.find({ teacherId: req.params.teacherId, status: { $in: ['available', 'booked'] } }).sort({ meetingDate: 1 });
    res.json(slots);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.put('/child/:studentId/authorized-pickup', auth, authorize(['parent']), async (req, res) => {
  try {
    ...
    const student = await Student.findByIdAndUpdate(req.params.studentId, { authorizedPickup }, { new: true });
    res.json({ message: 'Authorized pickup updated', student });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// ===== MySQL/Prisma-replaced routes =====
...
router.get('/pickup-requests', auth, authorize(['parent']), async (req, res) => {
  try {
    const requests = await query('SELECT pr.*, s.id AS studentRelId, s.firstName AS studentFirstName, s.lastName AS studentLastName FROM PickupRequest pr LEFT JOIN Student s ON pr.studentId = s.id WHERE pr.requestedById = ? ORDER BY pr.createdAt DESC', [req.userId]);
...
router.get('/ptm/slots', auth, authorize(['parent']), async (req, res) => {
  try {
    ...
    const slots = await query(
      `SELECT sl.*, ss.id AS sessionId, ss.sessionDate, ss.location, ss.notes AS sessionNotes, ss.teacherId, ss.batchId
       FROM PTMSlot sl
